...
void tstFail(const struct TstSourceLocation* where, const char* msg, ...)
{
   va_list args;
   va_start(args, msg);
   if (where->func == NULL || strcmp(tstCurrent, where->func) == 0) {
      tstPrintf(-1, "%s:%d:: error: TEST FAILED: %s\n",
         where->file, where->line, tstCurrent);
   } else {
...
static void CatchAbortHandler(const struct MtxErrorInfo* err)
{
   if (tstAbortState.enabled) {
      tstAbortState.enabled = 0;        // prevent recursion
      // Destroy all objects allocated since entering ASSERT_ABORT()
      mmRollback(tstAbortState.mmCheckpoint);
...
void tstPrepareCatchAbort(const struct TstSourceLocation* where, const char *estr)
{
   ...
   tstAbortState.mmCheckpoint = mmCheckpoint();
}
...
#error MTX_ZZZ undefined
   ...
   tstMessageThreshold = logGetDefaultThreshold();
   logSetDefaultThreshold(0);
