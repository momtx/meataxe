...
static void testprim()
{
   ...
   for (i = 0; i < Q - 1; i++) {
      if (a[i] != 1) {
         fprintf(stderr,"bigmktab: polynomial is not primitive!\n");
         exit(1);
      }
   }
   free(a);
...
static void writeHeader()
{
   ...
   if (fd == NULL) {
      perror(fname);
      fprintf(stderr,"Cannot create table file %s (%s)\n", fname, strerror(errno));
      exit(EXIT_ERR);
   }
   ...
   MESSAGE(1,("ZZZ version : %hu\n",header[0]));
   MESSAGE(1,("Field order : %hu=%hu^%hu\n",header[2],header[1],N));
   if (P != Q) {
...
{
   if (fwrite(&Minusone,sizeof(short),1,fd) != 1 ||
       fwrite(inc,sizeof(short),(size_t)Q - 1,fd) != (size_t) Q - 1 ||
       fwrite(FfToIntTable,sizeof(uint16_t),(size_t)Q,fd) != (size_t) Q ||
       fwrite(FfFromIntTable,sizeof(uint16_t),(size_t)Q,fd) != (size_t) Q ||
       fwrite(&numberOfSubfields,sizeof(uint16_t),1,fd) != 1 ||
       fwrite(subfieldOrders, sizeof(uint16_t), numberOfSubfields, fd) != numberOfSubfields ||
       fwrite(embeddingTables, sizeof(uint16_t), embeddingTablesSize,
               fd) != embeddingTablesSize ||
       ferror(fd)) {
